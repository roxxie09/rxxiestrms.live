...
new_ad_code = """<div id="awn-z10753238"></div>
...
for html_file in root.rglob("*.html"):
    ...
    # Skip if new ad div already exists
    if "awn-z10753238" in text:
        print(f"Already has new ads: {html_file}")
        continue
    
    # Find position: right after bootstrap script, before </body> (exact match to your original)
    bootstrap_pos = text.rfind('src="js/bootstrap.bundle.min.js"')
    body_pos = text.lower().rfind("</body>")
    
    if bootstrap_pos != -1 and body_pos != -1 and bootstrap_pos < body_pos:
        # Insert right after bootstrap script (same spot as old ads)
        insert_pos = text.find(">", bootstrap_pos) + 1
        new_text = text[:insert_pos] + "\n    " + new_ad_code + "\n" + text[insert_pos:]
    elif body_pos != -1:
        # Fallback: before </body>
        new_text = text[:body_pos] + new_ad_code + "\n" + text[body_pos:]
    else:
        print(f"No good insertion point: {html_file}")
        continue
    
    html_file.write_text(new_text, encoding="utf-8")
    print(f"Added new ads: {html_file}")
