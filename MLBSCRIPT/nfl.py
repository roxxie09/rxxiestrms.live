...
TEAM_NORMALIZATION_MAP = {
    ...
    "miami dolphins vs new york": "miami dolphins vs new york jets",
}
...
def parse_m3u_nfl_sd_section(m3u_text: str):
    """
    Parse the M3U and extract (title, url) pairs only from the NFL 01–16 section
    (between the first Dallas vs Detroit and the next --- NEXT SECTION ---).
    """
    ...
    for line in lines:
        if line.startswith("#EXTINF"):
            # get text after the comma
            title_part = line.split(",", 1)[1] if "," in line else ""

            # Turn on the NFL SD section when first known marker appears
            if ("NFL 01:" in title_part
                and "Dallas Cowboys vs Detroit Lions" in title_part):
                in_nfl_sd = True

            # Turn off when reaching the next section *after* NFL SD
...
def extract_nfl_index_from_href(href: str) -> int:
    ...
    # default to 1 if no explicit number (like redzone, if you ever use it)
    return 1
...
def build_ffmpeg_command(stream_url: str, display_name: str, nfl_index: int) -> str:
    ...
    - stream_url: M3U game URL (from IPTV server)
    - display_name: original 'Seattle Seahawks vs Atlanta Falcons'
    - nfl_index: X from nfl-streams-X
    """
...
def main():
    ...
    # 4) Walk the table rows IN ORDER and:
    #    - update hrefs in HTML
    #    - build ordered list for txt mapping
    ...
    # 5) Write mapping to a text file in the SAME order as nfl.html
