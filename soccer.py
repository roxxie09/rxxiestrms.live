...
INPUT_FILE = "soccer.html"
OUTPUT_FILE = "soccer.html"

def parse_event_time(event_time_str: str) -> datetime | None:
    # Your HTML uses: January 01, 2026 04:30 AM
    formats = [
        '%B %d, %Y %I:%M %p',   # January 01, 2026 04:30 AM
        '%B %d, %Y %H:%M %p',   # fallback if 24‑hour sneaks in with AM/PM
        '%B %d, %Y %H:%M'       # plain 24‑hour, no AM/PM
    ]
    for fmt in formats:
        try:
...
for row in rows:
    ...
    # Second <td> is the visible start time text
...
print("soccer.html updated successfully.")
